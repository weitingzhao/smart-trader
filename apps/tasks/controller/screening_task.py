...
class ScreeningTask(BaseTask):
    ...
    def _worker_run(self, script_name: str, logic : Logic, task_result: TaskResult, meta: dict, args: str = None):
        if script_name == 'dry run':
            self.fetching_snapshot(dry_run=True)
        elif script_name == 'Import':
            self.fetching_snapshot(dry_run=False)


    def fetching_snapshot(self,  dry_run):

        # Step 1.  Get the screening operations
        operations = instance.service.fetching().screening().screening_operation()
        ...
        for operation_job in operations:
            ...
            operation_job.status = ScreeningOperationChoices.DONE
