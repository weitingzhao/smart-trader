...
class UtilitiesLookupViewSet(viewsets.ModelViewSet):
    ...
    def category(self, request, category=None):
        types = UtilitiesLookup.objects.filter(category=category.replace('-','/')).values('type').annotate(total=Count('id'))
    ...
    def type(self, request, category=None, type=None):
        types = UtilitiesLookup.objects.filter(category=category.replace('-','/'), type=type.replace('-','/')).values('key', 'value')
