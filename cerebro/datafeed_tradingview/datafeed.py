...
from cerebro import datafeed_tradingview
...
class TvDatafeedLive(datafeed_tradingview.TvDatafeed):
    """                 
    ...
    def new_seis(self, symbol, exchange, interval, timeout=-1): 
        ...
        new_seis= datafeed_tradingview.Seis(symbol, exchange, interval)
    ...
    def new_consumer(self, seis, callback, timeout=-1):
        ...
        consumer= datafeed_tradingview.Consumer(seis, callback)
        if self._lock.acquire(timeout=timeout) is False:
            return False
    ...
    def get_hist(self,
                 symbol: str,
                 exchange: str = "NSE",
                 interval: datafeed_tradingview.Interval = datafeed_tradingview.Interval.in_daily,
                 n_bars: int = 10,
